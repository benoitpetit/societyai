...
SocietyAI is a **model-agnostic** multi-agent orchestration framework with zero
runtime dependencies. Its architecture is built upon a powerful and flexible
...
At the heart of SocietyAI lies an execution engine based on graphs. When a
workflow is executed, it is converted into a directed graph (DAG or cyclic)
where:
...
SocietyAI provides **two distinct levels of abstraction** for building
...
- **Features**: Fluent builder functions, direct execution, built-in patterns,
  middleware support
...
- **Building Blocks**: `GraphNode` with `NodeType` (10 types available)
- **Node Types**: `START`, `END`, `AGENT`, `HUMAN`, `PARALLEL`, `AGGREGATE`,
  `CONDITION`, `TRANSFORM`, `LOOP`, `COLLABORATIVE`
- **Best For**: Complex patterns (cycles, transforms, custom aggregations)
- **Learning Curve**: 🟡 Medium-High

> **💡 Decision Guide**: Start with the High-Level API. Only use the Low-Level
> API when you need features like:
>
...
When you use the High-Level API, `SocietyExecutor` automatically converts your
workflow configuration into a graph:
...
| `human`           | `START` → `HUMAN` → `END`                        | Pauses for human input, resumes when provided                   |
...
Society.create()
  ...
  .addAgent(agent2)
  .addTask(
    (s) => s.withId('analyze').withAgents(['agent1', 'agent2']).parallel() // TaskExecutionType
  );
...
- ⚠️ The high-level API is automatically converted to a graph by the
  `SocietyExecutor`
- ⚠️ For advanced scenarios (cycles, transforms, aggregations), use
  `GraphBuilder` directly
- ⚠️ You cannot mix high-level and low-level APIs in the same workflow (choose
  one approach)

### Advanced Routing Conversion

The `SocietyExecutor` handles complex routing scenarios when converting
workflows to graphs:
...
When you use `.thenResolve()`, the executor creates an intermediate CONDITION
node:

```typescript
// Workflow with dynamic routing
Society.create().addTask((t) =>
  t.withId('classify').thenResolve((results) => {
...
When you use `.withBranch()` or `.withConditionalNext()`, conditional edges are
created:
...
In permissive mode (default), tasks without explicit routing are automatically
chained:
...
**Strict Routing Mode**: When enabled, all intermediate tasks MUST define
explicit routing:
...
The execution engine supports 10 node types for building complex workflows:
...
Executes a single agent with native support for the **ReAct loop** (Reason +
Act).
...
3. **Validation**: If an output schema is defined, the response is validated via
   JSON Schema.
4. **Memory Storage**: The interaction is saved to long-term memory.

#### 3. **HUMAN** (Human-in-the-Loop)

Pauses the workflow execution and waits for external human input before
continuing.

**Workflow Mapping**: `human` → A `HUMAN` node (set via `.isHuman()` on a task).

**Behavior**:

1. The engine serializes the current state.
2. Execution pauses and returns a suspension signal.
3. The workflow can be resumed later via `engine.resume()` with the human's
   input.

**Use Cases**: Approval gates, manual review, interactive workflows.
...
#### 6. **COLLABORATIVE** (Collaboration Loop)
...
2. **Explicit Targeting**: Agents can address specific agents using
   `@agentId: message`.
...
The system will automatically extract the recipient and filter the message
history for each agent.

#### 7. **LOOP** (Iterative Loop)

Repeats the execution of a subgraph with a termination condition.

**Workflow Mapping**: Basic loops are supported via
`.withLoop(maxIterations, completionCondition)` on tasks. For complex loop
structures, use `GraphBuilder` directly.

#### 8. **AGGREGATE** (Aggregation)
...
Agent roles support customizable prompt templates with the following
placeholders:
...
Via `EngineAsModel`, you can encapsulate a complete execution graph as if it
were a simple AI model, allowing you to create orchestration hierarchies.
...
**Note**: SocietyAI is designed to be **model-agnostic**. You can use any AI
model by implementing the `AIModel` interface.
