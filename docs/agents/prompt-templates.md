...
This guide explains how to customize agent prompts using the template system in SocietyAI.

---

## 📝 Overview

Every agent in SocietyAI uses a **prompt template** to format the input before sending it to the AI model. The template system supports **placeholders** that are replaced with contextual data at runtime.
...
```typescript
const role = createRole('translator')
...
const agent = createAgent('assistant', role, model)
  .withMemory(memory);
```
...
const role = createRole('researcher')
...
const agent = createAgent('researcher', role, model)
...
```typescript
const role = createRole('debater')
  .withSystemPrompt('You are participating in a debate')
  .withPromptTemplate(`
{system}
...
const role = createRole('assistant')
  .withPromptTemplate('Lang: {context.lang}\n{input}');
...
For testing your custom templates, you can create test agents and verify the final prompts through the execution flow:
...
- **Learn more about roles**: [Role Builder Guide](role-builder-guide.md)
- **Add memory to agents**: [Memory System Guide](./capabilities/memory.md)
- **Enable tool calling**: [Tools Guide](./capabilities/tools.md)
- **Builders Reference**: [Agent Builder](./builders/agent-builder.md)
...
- **Issue Tracker**: [GitHub Issues](https://github.com/benoitpetit/societyai/issues)
- **Discussions**: [GitHub Discussions](https://github.com/benoitpetit/societyai/discussions)
- **Examples**: [src/__tests__/examples/](../src/__tests__/examples/)
