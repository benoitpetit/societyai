...
export class SocietyGraph {
      ...
      switch (node.type) {
        case NodeType.AGENT:
          if (!node.agentId) {
            throw new ProcessingFailedError(
              `AGENT node ${node.id} must have agentId`
            );
          }
          break;
        case NodeType.PARALLEL:
          if (!node.agentIds || node.agentIds.length === 0) {
            throw new ProcessingFailedError(
              `PARALLEL node ${node.id} must have agentIds`
            );
          }
          break;
  ...
  private async executeParallelNode(
    ...
    await Promise.all(
      nodeAgents.map((agent) => pool.submit(async () => {
        const prompt = this.buildPrompt(agent, context.currentResult, context);
        const result = await agent.model.process(prompt, context.signal);
...
export class GraphBuilder {
  ...
  addNode(
    id: string,
    type: NodeType,
    config?: Partial<Omit<GraphNode, 'id' | 'type'>>
  ): this {
  ...
  addEdge(
    from: string,
    to: string,
    config?: Partial<Omit<GraphEdge, 'from' | 'to'>>
  ): this {
