...
export class PipelineBuilder {
  ...
  scatterGather(
    ...
    return this.scatter(agentIds, name ? `${name}-scatter` : undefined)
      .gather(strategy, name ? `${name}-gather` : undefined);
  }
  ...
  onlyIf(condition: (ctx: PipelineContext) => boolean, agentIds: string[], name?: string): this {
    return this.when(
      condition,
      (builder) => builder.chain(agentIds),
      undefined,
      name
    );
  }
...
export const Pipelines = {
   ...
   */
  chain: (...agentIds: string[]) => 
    PipelineBuilder.create('chain').chain(agentIds).build(),

  /**
   * Scatter-gather pipeline
   */
  scatterGather: (agentIds: string[], strategy?: AggregationStrategy) =>
    PipelineBuilder.create('scatter-gather')
      .scatterGather(agentIds, strategy)
      .build(),
  ...
  expertRouting: (
    ...
    PipelineBuilder.create('expert-routing')
      .routeByContent(routes, defaultAgentId)
      .build(),
...
export function composePipelines(...pipelines: Pipeline[]): Pipeline {
  ...
  return {
    ...
    getSteps: () => allSteps,
    execute: async (input, agents, signal) => {
  ...
  return {
    ...
    execute: async (input, agents, signal) => {
      ...
      const results = await Promise.all(
        pipelines.map((p) => p.execute(input, agents, signal))
      );
