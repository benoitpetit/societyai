...
export const Middlewares = {
   ...
   */
  logging: (options?: { prefix?: string; logInput?: boolean; logOutput?: boolean }): Middleware => ({
    name: 'logging',
    ...
    fn: async (ctx, next): Promise<MiddlewareResult> => {
      try {
        ...
        const output = typeof fallbackValue === 'function' 
          ? fallbackValue(error as Error) 
          : fallbackValue;
        return { output, continue: false, metadata: { fallback: true, error: (error as Error).message } };
      }
    },
  }),
...
export class InMemoryMetricsCollector implements MetricsCollector {
  ...
  getHistogram(name: string): { count: number; sum: number; avg: number; min: number; max: number } {
    const values = this.histograms.get(name) ?? [];
    if (values.length === 0) {
  ...
  getAll(): { counters: Record<string, number>; histograms: Record<string, number[]>; gauges: Record<string, number> } {
    return {
...
export const StepMiddlewares = {
   ...
   */
  enrichResults: (enricher: (result: StepResult, ctx: StepMiddlewareContext) => StepResult): StepMiddleware => ({
    name: 'enrichResults',
    fn: async (ctx, next): Promise<StepResult[]> => {
      const results = await next();
