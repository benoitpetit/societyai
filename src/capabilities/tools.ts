...
export class ToolRegistry {
   ...
   */
  private validateParameters(params: Record<string, unknown>, schema: ToolParameterSchema): void {
    ...
    if (schema.required) {
      for (const required of schema.required) {
        if (!(required in params)) {
          throw new ProcessingFailedError(`Missing required parameter: ${required}`);
        }
      }
    }
   ...
   */
  private validateValue(value: unknown, schema: ToolParameterSchema, path: string): void {
    // Type validation
    ...
    if (schema.type === 'number' && typeof value === 'number') {
      if (schema.minimum !== undefined && value < schema.minimum) {
        throw new ProcessingFailedError(`Parameter ${path}: must be >= ${schema.minimum}`);
      }
      if (schema.maximum !== undefined && value > schema.maximum) {
        throw new ProcessingFailedError(`Parameter ${path}: must be <= ${schema.maximum}`);
      }
    }
    ...
    if (schema.type === 'string' && typeof value === 'string') {
      if (schema.minLength !== undefined && value.length < schema.minLength) {
        throw new ProcessingFailedError(`Parameter ${path}: length must be >= ${schema.minLength}`);
      }
      if (schema.maxLength !== undefined && value.length > schema.maxLength) {
        throw new ProcessingFailedError(`Parameter ${path}: length must be <= ${schema.maxLength}`);
      }
    }
