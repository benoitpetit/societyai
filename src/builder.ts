/**
 * @fileoverview Fluent Builder API for SocietyAI
 *
 * This module provides a comprehensive, model-agnostic, domain-independent
 * builder pattern for configuring AI agent societies, pipelines, and workflows.
 *
 * Design principles:
 * - Model-agnostic: Works with any AI model implementing AIModel interface
 * - Domain-independent: No hardcoded prompts or business logic
 * - Zero runtime deps: Pure TypeScript, no external dependencies
 ...
 *       .withCapabilities(['data-analysis', 'visualization']))
 *     .withModel(myModel))
 *   .addAgent(agent => agent
 *     .withId('reviewer')
 *     .withModel(myModel))
 *   .usePipeline(pipeline => pipeline
 *     .scatter(['analyst', 'reviewer'])
 *     .gather(Strategy.merge()))
 *   .build();
 * ```
 */
...
import { DefaultWorkflowExecutor } from './society';
...
export class FluentAgentBuilder {
  ...
  /**
   * Set the role using a builder function
   */
  withRole(builderFn: (builder: FluentRoleBuilder) => FluentRoleBuilder): this {
    const builder = new FluentRoleBuilder();
    this._role = builderFn(builder).build();
    return this;
  }
  ...
  build(): AgentConfig & { tags?: string[]; metadata?: Record<string, unknown> } {
    ...
    return {
      ...
      role: this._role,
      model: this._model,
      canCommunicateWith: this._canCommunicateWith.length > 0 ? this._canCommunicateWith : undefined,
      priority: this._priority,
      initialContext: Object.keys(this._initialContext).length > 0 ? this._initialContext : undefined,
...
export class FluentStepBuilder {
   ...
   */
  build(): WorkflowStep & { timeout?: number } {
    ...
    if (this._agentIds.length === 0) throw new InvalidConfigurationError('Step must have at least one agent');
...
export const AggregationStrategies = {
   ...
   */
  first: () => (results: StepResult[]): string => {
   ...
   */
  last: () => (results: StepResult[]): string => {
   ...
   */
  best: (scorer: (result: StepResult) => number) => (results: StepResult[]): string => {
   ...
   */
  reduce: <T>(
    reducer: (acc: T, result: StepResult) => T,
    initial: T,
   ...
   */
  structured: (format: 'json' | 'markdown' | 'list' = 'markdown') => (results: StepResult[]): string => {
    const successful = results.filter((r) => r.success);
...
export class Society {
    ...
    this._steps = [
      {
        ...
        agentIds,
        executionType: 'parallel',
        resultTransformer: aggregator as ((results: StepResult | StepResult[]) => unknown) | undefined,
      },
    ];
    return this;
  }
  ...
  build(): WorkflowConfig {
    ...
    if (!this._name) this._name = this._id;
    if (this._agents.length === 0) throw new InvalidConfigurationError('Society must have at least one agent');
    if (this._steps.length === 0) throw new InvalidConfigurationError('Society must have at least one step');
