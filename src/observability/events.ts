...
export class SocietyEventEmitter {
   ...
   */
  off<K extends keyof SocietyEventMap>(type: K, handler: EventHandler<SocietyEventMap[K]>): void {
   ...
   * Wait for an event
   */
  waitFor<K extends keyof SocietyEventMap>(type: K, timeout?: number): Promise<SocietyEventMap[K]> {
...
export class EventAggregator {
  ...
  getSummary(): EventSummary {
    ...
    const errors = this.events.filter(
      (e): e is WorkflowErrorEvent | StepErrorEvent | AgentErrorEvent => e.type.endsWith(':error')
    );
